...
%
% Summary: 
% Figure 2a and 2b in Zimmerman et al. 2024. Plots analytic solution and 
...
%          
% Output:
%   - Figure 2a and 2b in Zimmerman et al. 2024
%
% -------------------------------------------------------------------------
...
figure(gyre_strength); clf
    ...
    if gyre_strength == 1 %only weak gyre has unstable solution
        ...
        Qlsn = Qsol3(index_lsn); %Finds Q at the lower saddle node
    ...
    plot(h1(index_lsn:end),Qsol3(index_lsn:end),'k','LineWidth',1.5) %plot negative stable solution
...
txt = '\Delta H';
text(0.05,5,txt,'FontSize', 16) %adds text label
...
xlim([0 12000]) %if timestep and/or hosing rate changed in 'Stochastic_sims_3box.m' change time to reflect this change
xticks(0:2000:12000) %sets ticks for time axis
...
L = H_bif_min; %index of first realization to reach Q=0
int = 10; %define lag
q_anom = Qvec_re(:,1:L+int)-Qdet(1:L+int); %get anomaly
...
bif_min = h1(H_bif_min); %select hosing level of first crossing of Q=0
plot_min = find(H_ind>-0.3,1,'first'); %find index of lower x limit for plotting
...
% plot([h1(H_bif_min)-0.01125 h1(H_bif_min)],[varian(end-3)+.1 varian(end-3)+.1],'k-|') 
% txt = 'W';
% text(h1(H_bif_min)-0.006,varian(end-3)+.15,txt)
...
yyaxis left %left axis for standard autocorrelation
...
yyaxis right %right axis for unbiased autocorrelatin
...
ylim([rhoa(plot_min)-.01 rhoa(end-3)+.05]) %y limit just above and below data range
ylabel('\rho_{a}','FontSize',16)

y1 = ylim; plot(bif_min*[1 1],y1,':','Color',[1 0 1],'linewidth',1) %plot vertical line where first realization crosses Q=0

xlim([-0.3 0.15])
set(bb, 'Position', [.62, 0.15, .31, .35]);
set(gca,'Ticklength',[0.01 0.025], 'FontSize', 14)
ax = gca;
ax.YColor = [0 .7 0];
